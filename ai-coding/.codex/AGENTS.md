...
- “凡是进入新的复杂任务阶段（即使已有旧计划），必须先调用 Sequential Thinking 输出本阶段的步骤，再开始执行。”
- “Sequential Thinking 负责阶段性规划，update_plan 只用来同步进度，不能替代前者。”
- 如有旧蓝图，也要在执行前，通过一次新的 Sequential Thinking 复核当前阶段的计划。
